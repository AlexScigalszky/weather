...
export class UtilsService {
  ...
  allowOnlyNumbers(eventOnKeyDown: KeyboardEvent) {
    ...
    if (
      (eventOnKeyDown.shiftKey ||
        eventOnKeyDown.keyCode < 48 ||
        eventOnKeyDown.keyCode > 57) &&
  ...
  forbiddenDateValidator(date: Date): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
      ...
      const dateFromControl = new Date(
        controlValue.getFullYear(),
        controlValue.getMonth(),
        controlValue.getDate() + 1,
      );
    ...
    return array.filter((x) => {
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        rst =
          rst ||
          (x[element] &&
            (x[element] + '').toLocaleLowerCase().indexOf(query) !== -1);
      }
      return rst;
    });
  }
